...
module.exports = function (graph) {
  ...
  function addRelationsNodeData (relations, direction) {
    return relations
      .filter((relation) => relation.type !== 'st:sameAs')
      .map((relation) => ({
        direction,
        ...relationNodeData(relation.id),
        ...relation
      }))
  }
  ...
  function makeObjectRelations (object) {
    ...
    // const incoming = addRelationsNodeData(graph.getIncoming(objectId), 'in')
    const outgoing = addRelationsNodeData(graph.getOutgoing(objectId), 'out')
